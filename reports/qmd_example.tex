...
\tightlist
\item
  \textbf{pandas} (McKinney 2010)
\item
  \textbf{altair} (VanderPlas 2018)
\item
  \textbf{click} (Team 2020)
\item
  \textbf{Quarto} (Allaire et al. 2022)
\end{itemize}

\emph{Note: this report is adapted from Timbers Timbers (2020).}
...
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{../results/horse_pops_plot.png}

}

\caption{\label{fig-provinces}Horse populations for all provinces in
Canada from 1906 - 1972.}

\end{figure}%

We can see from Figure~\ref{fig-provinces} that Ontario, Saskatchewan
...
Table~\ref{tbl-provinces}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{horses\_sd\_table }\OperatorTok{=}\NormalTok{ pd.read\_csv(}\StringTok{"../results/horses\_sd.csv"}\NormalTok{)}
\NormalTok{largest\_sd }\OperatorTok{=}\NormalTok{ horses\_sd\_table[}\StringTok{\textquotesingle{}Province\textquotesingle{}}\NormalTok{].values[}\DecValTok{0}\NormalTok{]}
\NormalTok{Markdown(horses\_sd\_table.to\_markdown(index }\OperatorTok{=} \VariableTok{False}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{longtable}[]{@{}lr@{}}

\caption{\label{tbl-provinces}Standard deviation of historical
...
Additionally, note that in Table~\ref{tbl-provinces} we consider the
sample standard deviation of the number of horses during the same time
span as Figure~\ref{fig-provinces}.

\begin{figure}[H]

\centering{

\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{../results/horse_pops_plot_largest_sd.png}

}

\caption{\label{fig-largest}Horse populations for the province with the
largest standard deviation}

\end{figure}%

In Figure~\ref{fig-largest} we zoom in and look at the province of
